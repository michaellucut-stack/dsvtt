...
type DirectorTab = 'npcs' | 'events' | 'secrets';
      ...
      >
        <div className="min-w-0">
          ...
          {!expanded && statsPreview && (
            <p className="truncate text-[10px] text-parchment-500 mt-0.5">
              {statsPreview}
            </p>
          )}
        </div>
            ...
            {editStats.map((stat, i) => (
              <div key={i} className="flex gap-1">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setEditStats((prev) => prev.filter((_, j) => j !== i))
                  }
                  className="shrink-0 text-charcoal-500 hover:text-crimson-400"
                >
...
function NpcsTab({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="flex flex-col gap-2 p-3">
      ...
      {showForm && (
        <NpcForm sessionId={sessionId} onDone={() => setShowForm(false)} />
      )}
      ...
      {activeSecretNote && (
        <div className="space-y-2 rounded-card border border-charcoal-700 bg-charcoal-800/30 p-3">
          <input
            ...
            onChange={(e) =>
              updateNote(activeSecretNote.id, { title: e.target.value })
            }
