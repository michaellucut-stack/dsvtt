...
export interface ClientToServerEvents {
  // Room events
  ROOM_CREATE: (payload: RoomCreatePayload, callback: (ack: { ok: boolean; roomId?: string; error?: string }) => void) => void;
  ROOM_JOIN: (payload: RoomJoinPayload, callback: (ack: { ok: boolean; error?: string }) => void) => void;
  ROOM_LEAVE: (payload: RoomLeavePayload) => void;

  // Game lifecycle events
  GAME_START: (payload: GameStartPayload, callback: (ack: { ok: boolean; sessionId?: string; error?: string }) => void) => void;
  ...
  TOKEN_ADD: (payload: TokenAddPayload, callback: (ack: { ok: boolean; tokenId?: string; error?: string }) => void) => void;
